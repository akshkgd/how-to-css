...
const items = [
  {
    ...
    desc: "Creamy tomato gravy with soft paneer cubes, flavored with Indian spices.",
    price: 249,
...
function App() {
  return (
    <> 
       {/* <h1 className="mt-24 text-center font-bold text-3xl">Welcome to react js! {Math.floor(Math.random()*100)} </h1>  */}
       <div className="flex gap-4 flex-wrap mt-24 max-w-6xl mx-auto justify-center">
          {
        items.map((item)=>{
          return <Card title={item.title} price={item.price} ratings={item.ratings} img={item.img} desc={item.desc} />
        })
       }
       </div>
